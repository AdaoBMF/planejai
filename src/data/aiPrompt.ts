...
export function buildPrompt(simulation: SimulationRecord) {
  const { income, expenses, debts, goalName, goalDeadline, goalAmount } =
    simulation
  ...
  Dados da simulação:
    ...
    - Prazo desejado: ${goalDeadline}
    - Economia mensal necessária para atingir a meta no prazo: ${monthlySavingsNeeded}
  ...
  Regras:
    ...
    - Não repita informações entre ações
    - Nunca use markdown dentro dos valores do JSON
    - Para o campo "feasibility status" use os seguinte critérios:
      ...
      - "unfeasible": salde negativo superior a 20% do valor da economia necessária
  `
}
